...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-packet-sys-arena-detail.ps1

Rscript tests/test_domain_review.R

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-packet-sys-arena-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-packet-sys-arena-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-packet_sys_arena_core-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
